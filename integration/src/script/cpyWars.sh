...
cd $SOURCE_WEBTIERSFEEDER_MODULE_RELATIVE_PATH
rm "$CATALINA_HOME/webapps/$SOURCE_WEBTIERSFEEDER_WAR_NAME" >> /dev/null 2>> "$INTEGRATION_HOME/$LOG_FILE"
rm -rf "$CATALINA_HOME/webapps/$SOURCE_WEBTIERSFEEDER_NAME" >> /dev/null 2>> "$INTEGRATION_HOME/$LOG_FILE"
cp "target/$SOURCE_WEBTIERSFEEDER_WAR_NAME" "$CATALINA_HOME/webapps/$SOURCE_WEBTIERSFEEDER_WAR_NAME" >> /dev/null 2>> "$INTEGRATION_HOME/$LOG_FILE"
